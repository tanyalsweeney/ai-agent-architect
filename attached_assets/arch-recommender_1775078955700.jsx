...
const getSecurityLevel = (selected = []) => {
...
// ─── API CALL ─────────────────────────────────────────────────────────────────
// ─── MOCK INFERENCE ──────────────────────────────────────────────────────────
// Parses the description locally to produce plausible inferences without an API
// call. Swap inferFromDescription for callClaude(buildInferencePrompt(desc))
// once API auth is wired up in production.
const inferFromDescription = (description) => {
const d = description.toLowerCase();
const has = (…terms) => terms.some(t => d.includes(t));
...
// ── UNIVERSAL BASELINE — always recommended, no exceptions ──
// Any agent system that will be shared, deployed, or run in production
// should have these. We default high and let the user remove them.
security.add(“mfa_rbac”);       // Identity is always required
security.add(“audit”);          // You always want to know what happened
security.add(“e2e_encryption”); // Data in motion and at rest should always be encrypted

// ── CONCURRENCY / SCALE — multiple concurrent users means stronger identity ──
// If more than one person or process touches this, access controls are essential.
...
// ── AUTONOMY — agents acting independently need stronger accountability ──
if (autonomy === “fully_autonomous” || autonomy === “approval_key”) {
security.add(“audit”);       // Non-negotiable when agents act without full HITL
security.add(“e2e_encryption”);
}

// ── MULTI-AGENT TOPOLOGY — distributed trust surfaces ──
if (topology === “orchestrator” || topology === “cooperative” ||
topology === “competing” || topology === “hybrid”) {
security.add(“mfa_rbac”);   // Agent-to-agent calls need identity
security.add(“audit”);      // Cross-agent actions need traceability
}
...
security.add(“audit”);
security.add(“e2e_encryption”);
security.add(“soc2”); // Enterprise customers almost always ask for SOC2
}
...
// ── EXPLICIT ADVANCED SECURITY MENTIONS ──
...
// Build a short summary from what we understood
const topologyLabel = { single: “single agent”, orchestrator: “orchestrator with subagents”, cooperative: “cooperative agent team”, pipeline: “sequential pipeline”, competing: “competing agents” }[topology] || “agent system”;
const platformLabel = platform ? `on ${platform.toUpperCase()}` : “”;
const summary = `A ${topologyLabel} ${platformLabel} that ${description.split(".")[0].toLowerCase().replace(/^(build|create|make|design|implement)\s+(a|an|the)?\s*/i, "").trim()}.`.replace(/\s+/g, “ “);
...
const STAGE_LABELS = { describe:“Describe”, refine:“Architecture”, constraints:“Constraints”, tools:“Tools”, security:“Security”, review:“Review” };

function BreadcrumbBar({ stage, setStageAndScroll }) {
...
function SecurityPanel({ selected, onChange }) {
...
function SecurityBanner({ selected }) {
const secLevel = getSecurityLevel(selected);
...
const setStageAndScroll = (s) => { setStage(s); window.scrollTo({ top: 0, behavior: “instant” }); };
...
const [toolSelections, setToolSelections] = useState(new Set());
...
if (d.inferred) setInferred(d.inferred);
};
...
setError(null);
// Simulate a brief analysis delay for UX, then run local inference
setTimeout(() => {
try {
const result = inferFromDescription(description);
setInferred(result);
const prepped = {};
Object.keys(FIELDS).forEach(k => { if (result[k]) prepped[k] = result[k]; });
setAnswers(prepped);
if (result.security?.length) setSecurity(result.security);
if (result.inferredTools?.length) setTools(result.inferredTools);
// Pre-compute tool selections so they’re ready when user reaches tools page
// We pass description and platform from result since answers aren’t set yet
setToolSelections(new Set());  // will be hydrated on tools page entry via useEffect pattern
...
const btnDisabled = { …btnPrimary, background: T.border, color: T.muted, cursor: “default”, opacity: 0.5 };
...
{coreFields.map(f => (
<FieldCard key={f} fieldKey={f} value={answers[f]} onChange={val => setField(f, val)} inferred={!!(inferred && inferred[f])} />
))}
...
// Infer which tools to pre-select based on description + inferred tools
const inferToolSelections = () => {
 const selected = new Set(tools); // start with whatever was already inferred
 const d = description.toLowerCase();
 const has = (...terms) => terms.some(t => d.includes(t));
 // Universal
 ...
 if (platform === "aws") {
   ...
   selected.add("aws_agentcore"); // always recommended for AWS agents
 }
 // Azure-specific
 if (platform === "azure") {
   ...
   selected.add("az_foundry"); // always recommended for Azure agents
 }
 // GCP-specific
 if (platform === "gcp") {
   ...
   selected.add("gcp_agent_engine"); // always recommended for GCP agents
   selected.add("gcp_adk");
 }
 return selected;
};

// Hydrate tool selections if empty (first visit)
...
const toggleTool = (val) => setToolSelections(s => { const n = new Set(s); n.has(val) ? n.delete(val) : n.add(val); return n; });

const sortedBySelected = (toolList) => [...toolList].sort((a, b) => (toolSelections.has(b.value) ? 1 : 0) - (toolSelections.has(a.value) ? 1 : 0));

const renderToolGrid = (toolList, sectionLabel, accentColor) => (
 <div style={{ marginBottom: "24px" }}>
...
const platformNames = { aws: "AWS Native", azure: "Azure Native", gcp: "GCP Native", onprem: "On-Premises" };
...
<button style={{ …btnGhost, width: “100%”, marginTop: “8px” }} onClick={() => { setStageAndScroll(“describe”); setDescription(””); setInferred(null); setAnswers({}); setSecurity([]); setTools([]); setToolSelections(new Set()); setConstraints(””); setCustomAgents(””); setLegacyDesc(””); }}>
Start over
</button>
</div>
</div>
